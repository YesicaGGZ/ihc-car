// === Config ===
const API_BASE = "http://98.88.113.82:5500/api"; // Ajusta según tu backend
...
// Botón de activación
const btnVoz = document.getElementById("toggle-voz-btn");
if (btnVoz) {
  let grabando = false;
  btnVoz.addEventListener("click", async () => {
    if (!grabando) {
      grabando = true;
      btnVoz.classList.replace("btn-warning", "btn-danger");
      btnVoz.innerHTML = '<i class="bi bi-stop-circle"></i> Detener voz';
      await startRecording();
    } else {
      grabando = false;
      btnVoz.classList.replace("btn-danger", "btn-warning");
      btnVoz.innerHTML = '<i class="bi bi-mic"></i> Probar por voz';
      await stopRecording();
    }
  });
}
